...
const socket = io("http://localhost:5000"); // Changed from 8000 to 5000
...
export default function CommunityForum() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get("http://localhost:5000/posts"); // Changed from 8000 to 5000
  ...
  const handleCreatePost = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:5000/posts", formData, { // Changed from 8000 to 5000
  ...
  const handleAddComment = async (postId, parentCommentId = null) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/posts/${postId}/comment`, { // Changed from 8000 to 5000
        text,
        parentCommentId,
      });
            ...
            posts.map((post) => (
              ...
              >
                <div className="p-5">
                  ...
                  {post.image && (
                    <img
                      src={`http://localhost:5000${post.image}`} // Changed from 8000 to 5000
                      alt={post.title}
